...
def test_console_scripts_point_at_domain_packages():
    ...
    assert scripts["agent-mgmt-smoke-agent"] == "agent_management.agents.smoke:main"
    assert scripts["agent-mgmt-agent-versioning"] == "agent_management.agents.versioning:main"
    assert scripts["agent-mgmt-sync-vqrs"] == "agent_management.semantic_views.vqrs:main"
    assert scripts["agent-mgmt-sv-scores"] == "agent_management.evals.sv_scores:main"
    assert scripts["agent-mgmt-format-sv-comment"] == "agent_management.evals.sv_comment:main"
    assert scripts["agent-mgmt-detect-sv-drift"] == "agent_management.semantic_views.drift:main"
    assert scripts["agent-mgmt-deploy-svs-yaml"] == "agent_management.semantic_views.deploy_yaml:main"
    assert scripts["agent-mgmt-regen-sv-gold"] == "agent_management.semantic_views.regen_gold:main"
    assert scripts["agent-mgmt-snapshot-agent"] == "agent_management.agents.snapshot:main"
