...
METRICS (
    ...
    STAFFING.OVERSTAFFED_COUNT AS COUNT(CASE WHEN NOT STAFFING.IS_UNDERSTAFFED AND STAFFING.COVERAGE_RATIO > 1 THEN 1 END)
      WITH SYNONYMS ('overstaffed', 'over_coverage_count')
      COMMENT = 'Count of overstaffed shifts (coverage > 100% and not flagged understaffed)',
...
WITH EXTENSION (CA = $$
{
  "module_custom_instructions": {
    ...
    "sql_generation": "Group by DEPARTMENT for department-level analysis. Use IS_UNDERSTAFFED filter for coverage issues. Calculate efficiency as ACTUAL_LABOR_HOURS / visitor_count when joined with visit data. Weekend and holiday staffing often differs significantly. Prefer the SEMANTIC_VIEW(...) form for direct metric questions and do NOT add extra context columns (start_date, end_date, shift_count, etc.) unless the user explicitly asks for them; return only the columns needed to answer the question."
  },
  "verified_queries": [
    {
    ...
    },
    {
      ...
      "sql": "SELECT * FROM SEMANTIC_VIEW(\n  {{ target.database }}.SEMANTIC.SEM_STAFFING_ANALYTICS\n  METRICS understaffed_count, overstaffed_count\n  DIMENSIONS job_role\n) ORDER BY understaffed_count DESC NULLS LAST",
      "verified_by": "Cortex Analyst",
      "verified_at": 1745200000,
      "use_as_onboarding_question": false
    },
