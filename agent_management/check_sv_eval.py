...
def get_sv_eval_data(cur, database: str, schema: str, sv_name: str, run_name: str) -> list[dict]:
    try:
        ...
        print(f"    ERROR querying eval data: {e}")
        return []
...
def main():
    ...
    print(f"Thresholds: sql_correctness >= {sv_threshold}, max_regressions <= {max_regressions}")
    print("=" * 60)
    ...
    for sv_name in sv_names:
        ...
        print(f"    Score: {metrics['score']:.4f} {'≥' if score_pass else '<'} {sv_threshold} [{'PASS' if score_pass else 'FAIL'}]")
        print(f"    Regressions: {metrics['regressions']} {'≤' if regression_pass else '>'} {max_regressions} [{'PASS' if regression_pass else 'FAIL'}]")
    ...
    print(f"\n{'=' * 60}")
    if total_checked == 0:
        print("NO EVAL DATA FOUND — cannot determine pass/fail")
        print("Run SV evaluations first, then re-run this check.")
        sys.exit(0)
    elif all_passed:
        print(f"SV EVAL GATE: PASSED ({total_checked} views checked)")
        sys.exit(0)
    else:
        print(f"SV EVAL GATE: FAILED ({total_checked} views checked)")
        sys.exit(1)
